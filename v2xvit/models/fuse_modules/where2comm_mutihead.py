...
class MutualCrossAttention(nn.Module):
    def __init__(self, num_channels,ratio=16):
        ...
        mid_channel = num_channels // ratio
        self.channel_att_module = ChannelAttention(num_channels)
        self.cbam = CBAM(num_channels)
        self.scale = ScaledDotProductAttention(num_channels)

    def forward(self, x):
        cav_num,c,h,w=x.shape
        spatial_att = self.cbam(x)
        channel_att = self.channel_att_module(x)
        channel_att = channel_att.view(cav_num, c, -1).permute(2, 0, 1)
        spatial_att = spatial_att.view(cav_num,c,-1).permute(2,0,1)
        cha_att = self.scale(channel_att,spatial_att,spatial_att)
        spa_att = self.scale(spatial_att,channel_att,channel_att)
        cha_att = cha_att.permute(1,2,0).view(cav_num,c,h,w)
        spa_att = spa_att.permute(1,2,0).view(cav_num,c,h,w)
        output = cha_att + spa_att
...
class MeanFusion(nn.Module):
    ...
    def forward(self, x):
        out = torch.mean(x, dim=0, keepdim=False)
        return out
...
class PositionAttentionModule(nn.Module):
    ...
    def __init__(self,d_model):
        ...
        self.mutual = MutualCrossAttention(d_model)
